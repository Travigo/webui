<template>
  <div class="rounded-xl border px-4 py-3 text-sm font-medium" :class="toneClass">
    <div class="flex items-start gap-3">
      <span v-if="resolvedIcon" class="material-symbols-outlined mt-0.5 text-[20px] leading-none">
        {{ resolvedIcon }}
      </span>
      <div class="min-w-0">
        <h2 v-if="title" class="font-bold">
          {{ title }}
        </h2>
...
export default {
  ...
  computed: {
    ...
    toneClass() {
      return {
        info: 'border-brand-blue/20 bg-brand-blue/10 text-brand-blue-dark dark:border-brand-blue/40 dark:bg-brand-blue/100/10 dark:text-brand-blue-light',
      ...
      }[this.type] || 'border-brand-blue/20 bg-brand-blue/10 text-brand-blue-dark dark:border-brand-blue/40 dark:bg-brand-blue/100/10 dark:text-brand-blue-light'
    }
  }
}
</script>
