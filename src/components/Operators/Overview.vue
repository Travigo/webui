  ...
  <div class="space-y-4 p-4 sm:p-5">
    <section class="grid gap-3 sm:grid-cols-2" v-if="contactItems.length > 0">
      <a
        ...
        class="flex items-center gap-3 rounded-xl bg-slate-50 p-3 transition hover:bg-brand-blue/10"
      >
        <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-brand-blue shadow-sm">
          <span class="material-symbols-outlined text-[22px]">{{ item.icon }}</span>
        </span>
        <span class="min-w-0 flex-1">
          <span class="block text-xs font-semibold text-slate-500">{{ item.label }}</span>
          <span class="block truncate text-sm font-bold text-slate-950">{{ item.value }}</span>
        </span>
        <span class="material-symbols-outlined text-[18px] text-slate-400">{{ item.external ? 'open_in_new' : 'chevron_right' }}</span>
      </a>
    </section>

    <section class="rounded-xl bg-slate-50 p-4" v-if="operator.OperatorGroup">
      ...
      <router-link
        ...
        class="mt-1 inline-flex items-center gap-1 text-base font-bold text-brand-blue transition hover:text-brand-blue"
      >
    ...
    <section class="rounded-xl bg-slate-50 p-4" v-if="operator.OtherNames?.length > 0">
    ...
    <section class="rounded-xl bg-amber-50 px-3 py-3 text-sm text-amber-800" v-if="contactItems.length === 0 && !operator.OperatorGroup && !operator.OtherNames?.length">
